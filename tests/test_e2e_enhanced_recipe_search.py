...
class TestE2EEnhancedRecipeSearch:
    ...
    async def test_complete_search_workflow_with_search_tags(self):
        ...
        **Cuisine:** italian
        ...
        with patch("src.makemyrecipe.services.anthropic_service.settings") as mock_settings:
            ...
            assert "carbonara" in recipe.title.lower(), "Recipe title should contain 'carbonara'"
            ...
            assert recipe.primary_source is not None, "Should have primary source citation"
            assert isinstance(recipe.primary_source, Citation), "Primary source should be Citation object"
            
            # Check that the recipe has proper timestamps and ID
            assert recipe.id is not None, "Recipe should have an ID"
            assert recipe.created_at is not None, "Recipe should have creation timestamp"
            ...
            assert mock_client.messages.create.call_count == 3, "Should make 3 API calls for search tag workflow"
    ...
    async def test_search_tag_extraction_and_processing(self):
        ...
        assert "</search>" not in cleaned_text, "Should remove search tags"
        assert "pasta carbonara authentic recipe" not in cleaned_text, "Should remove search content"
        assert "I'll help you with that recipe!" in cleaned_text, "Should preserve other content"
    ...
    async def test_recipe_conversion_with_citations(self):
        ...
        from src.makemyrecipe.services.recipe_service import RecipeResult, RecipeMetadata, DifficultyLevel, CuisineType
        ...
        additional_citation = Citation(
            title="Another Recipe Source",
            url="https://another.com/recipe"
        )
